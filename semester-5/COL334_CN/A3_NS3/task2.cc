...
{ 
  ...
  app->Setup (ns3TcpSocket, sinkAddress, 3000, 200000, DataRate ("3Mbps"));
  nodes.Get (0)->AddApplication (app);
  app->SetStartTime (Seconds (1.));
  app->SetStopTime (Seconds (40.));

  AsciiTraceHelper asciiTraceHelper;
  Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("task2.cwnd");
  ...
  anim.SetConstantPosition(nodes.Get(0) ,1.0 , 75.0);
  anim.SetConstantPosition(nodes.Get(1) ,100.0 , 75.0);
  Simulator::Run ();
  Simulator::Destroy ();
