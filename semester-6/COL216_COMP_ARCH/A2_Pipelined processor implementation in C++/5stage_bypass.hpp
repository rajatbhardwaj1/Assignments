...
struct MIPS_Architecture
{
    ...
    */
    ...
    void executeCommandspipelined()
    {
        ...
        cur_state.ins_dec.bad1 = 0;
        cur_state.ins_dec.bad2 = 0;
        cur_state.ins_dec.badmem = 0;
        ...
        while (PCcurr < commands.size())
        {
            ...
            new_state.ins_dec.bad1 = 0;
            new_state.ins_dec.bad2 = 0;
            new_state.ins_dec.badmem = 0;
            ...
            if (!cur_state.ins_mem.nop)
            {
                ...
                else if (cur_state.ins_exec.write_mem)
                {
                    ...
                    data[cur_state.ins_exec.exe_result_op] = registers[registerMap[new_state.ins_mem.reg_write]];
            ...
            if (!cur_state.ins_exec.nop)
            {
                ...
                bool shoudstall = false ;
                if (cur_state.ins_dec.bad1)
                {
                    //std::cout << "reg1 garbage\n";
                    // new_state.ins_alu.exe_result_op = instructions[cur_state.ins_read_reg.operation](*this, cur_state.ins_mem_read.mem_result_op, cur_state.ins_read_reg.value2, cur_state.ins_read_reg.goto_addr);
                    if (cur_state.ins_exec.reg_write.compare(cur_state.ins_dec.rubbish1) == 0 && !(cur_state.ins_exec.use_mem && cur_state.ins_exec.write_reg))
                    {
                        cur_state.ins_dec.value1 = cur_state.ins_exec.exe_result_op;
                        //std::cout << "Using " << cur_state.ins_dec.value1 << " instead from previous ALU output\n";
                        cur_state.ins_dec.bad1 = 0;
                    }
                    else if (cur_state.ins_mem.reg_write.compare(cur_state.ins_dec.rubbish1) == 0 && !(cur_state.ins_mem.use_mem && cur_state.ins_mem.write_reg))
                    {
                        cur_state.ins_dec.value1 = cur_state.ins_mem.exe_result_op;

                        //std::cout << "Using " << cur_state.ins_dec.value1 << " instead from previous MEM ALU op\n";
                        cur_state.ins_dec.bad1 = 0;
                    }
                    else if (cur_state.ins_mem.reg_write.compare(cur_state.ins_dec.rubbish1) == 0)
                    {
                        cur_state.ins_dec.value1 = cur_state.ins_mem.mem_result_op;

                        //std::cout << "Using " << cur_state.ins_dec.value1 << " instead from previous MEM output\n";

                        //std::cout << "Write back enable = " << new_state.ins_exec.write_reg << '\n';
                        cur_state.ins_dec.bad1 = 0;
                    }
                    else
                    {
                        new_state.ins_exec.nop = 0;
                        shoudstall = true ; 
                    }
                }
                if (cur_state.ins_dec.bad2)
                {
                    //std::cout << "reg2 garbage\n";
                    // new_state.ins_alu.exe_result_op = instructions[cur_state.ins_read_reg.operation](*this, cur_state.ins_mem_read.mem_result_op, cur_state.ins_read_reg.value2, cur_state.ins_read_reg.goto_addr);
                   if (cur_state.ins_exec.reg_write.compare(cur_state.ins_dec.rubbish2) == 0 && !(cur_state.ins_exec.use_mem && cur_state.ins_exec.write_reg))
                    {
                        cur_state.ins_dec.value2 = cur_state.ins_exec.exe_result_op;
                        //std::cout << "Using " << cur_state.ins_dec.value2 << " instead from previous ALU output\n";
                        cur_state.ins_dec.bad2 = 0;
                    }
                    else if (cur_state.ins_mem.reg_write.compare(cur_state.ins_dec.rubbish2) == 0 && !(cur_state.ins_mem.use_mem && cur_state.ins_mem.write_reg))
                    {
                        cur_state.ins_dec.value2 = cur_state.ins_mem.exe_result_op;

                        //std::cout << "Using " << cur_state.ins_dec.value2 << " instead from previous MEM ALU op\n";
                        cur_state.ins_dec.bad2 = 0;
                    }
                    else if (cur_state.ins_mem.reg_write.compare(cur_state.ins_dec.rubbish2) == 0)
                    {
                        cur_state.ins_dec.value2 = cur_state.ins_mem.mem_result_op;

                        //std::cout << "Using " << cur_state.ins_dec.value2 << " instead from previous MEM output\n";
                    ...
                    // new_state.ins_alu.exe_result_op = instructions[cur_state.ins_read_reg.operation](*this, cur_state.ins_read_reg.value1, cur_state.ins_mem_read.mem_result_op, cur_state.ins_read_reg.goto_addr);
                }
                if (shoudstall)
                {
                    goto stall1 ;
            ...
            if (!cur_state.ins_dec.nop)
            {
                ...
                if (i_type.find(new_state.ins_dec.Ins[0]) != i_type.end())
                {
                    ...
                    if (reg_lock[new_state.ins_dec.Ins[2]] != 0)
                    {

                        new_state.ins_dec.bad1 = 1;
                        new_state.ins_dec.rubbish1 = new_state.ins_dec.Ins[2];
                    }
                ...
                else if (branch.find(new_state.ins_dec.Ins[0]) != branch.end())
                {
                    ...
                    if (reg_lock[new_state.ins_dec.Ins[1]] != 0 || reg_lock[new_state.ins_dec.Ins[2]] != 0)
                    {
                        ...
                        if (reg_lock[new_state.ins_dec.Ins[1]])
                        {

                            new_state.ins_dec.bad1 = 1;
                            new_state.ins_dec.rubbish1 = new_state.ins_dec.Ins[1];
                        }
                        if (reg_lock[new_state.ins_dec.Ins[2]])
                        {

                            // bypass
                            new_state.ins_dec.bad2 = 1;
                            new_state.ins_dec.rubbish2 = new_state.ins_dec.Ins[2];
                        }
                    }
                ...
                else if (r_type.find(new_state.ins_dec.Ins[0]) != r_type.end())
                {
                    ...
                    if (reg_lock[new_state.ins_dec.Ins[2]] != 0 || reg_lock[new_state.ins_dec.Ins[3]] != 0)
                    {
                        ...
                        if (reg_lock[new_state.ins_dec.Ins[3]])
                        {

                            // bypass
                            new_state.ins_dec.bad2 = 1;
                            new_state.ins_dec.rubbish2 = new_state.ins_dec.Ins[3];
                        }
                    }
                    // destination !!!!!!??
                    //std:://std::cout << "garbaged " << new_state.ins_dec.bad1 << " " << new_state.ins_dec.bad2 << '\n';
                    reg_lock[new_state.ins_dec.Ins[1]] += 1;
                }
                else if (lwsw.find(new_state.ins_dec.Ins[0]) != lwsw.end())
                {
                    ...
                    if (new_state.ins_dec.Ins[0] == "lw")
                    {
                        ...
                        {
                            //std::cout << "stalling!!\n";
                            if (reg_lock[extractreg(new_state.ins_dec.Ins[2])])
                            {

                                new_state.ins_dec.bad1 = 1;
                                new_state.ins_dec.rubbish1 = extractreg(new_state.ins_dec.Ins[2]);
                            }
                        }
                        reg_lock[new_state.ins_dec.Ins[1]] += 1;
                    }
                    if (new_state.ins_dec.Ins[0] == "sw")
                    {
                        ...
                        if (reg_lock[new_state.ins_dec.Ins[1]] != 0 || reg_lock[extractreg(new_state.ins_dec.Ins[2])] != 0)
                        {
                            ...
                            if (reg_lock[new_state.ins_dec.Ins[1]])
                            {
                                ...
                                new_state.ins_dec.badmem = 1;
                                new_state.ins_dec.rubbishmem = new_state.ins_dec.Ins[1];
                            }
                        }
                    }
        ...
        stall1:
            ...
            new_state.ins_exec = cur_state.ins_exec;
            new_state.ins_mem.nop = 1;
