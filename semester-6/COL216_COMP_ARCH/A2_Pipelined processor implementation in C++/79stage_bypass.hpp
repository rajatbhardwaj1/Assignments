...
struct MIPS_Architecture
{
	...
	*/
	...
	void executeCommandspipelined()
	{
		...
		cur_state.ins_alu.bad1 = 0;
		cur_state.ins_alu.bad2 = 0;
		cur_state.ins_alu.badmem = 0;
		...
		cur_state.ins_mem_write.badmem = 0;
		...
		while (PCcurr < commands.size())
		{
			...
			new_state.ins_alu.bad1 = 0;
			new_state.ins_alu.bad2 = 0;
			new_state.ins_alu.badmem = 0;
			...
			new_state.ins_mem_write.badmem = 0;
			...
			if (!cur_state.ins_mem_write.nop)
			{
				...
				{
					if (cur_state.ins_mem_write.badmem)
					{
						// std::cout<<"Register "<<cur_state.ins_mem_write.rubbishmem<<" was bad\n";
						if (reg_lock[cur_state.ins_mem_write.rubbishmem] == 0)
						{
							cur_state.ins_alu.mem_write_value = registers[registerMap[cur_state.ins_mem_write.rubbishmem]];
							cur_state.ins_mem_write.badmem = 0;
						}
						else if (cur_state.ins_mem_write.destination == cur_state.ins_mem_write.rubbishmem)
						{
							cur_state.ins_alu.mem_write_value = new_state.ins_mem_read.mem_result_op;
							cur_state.ins_mem_write.badmem = 0;
						}
						else
						{
			...
			if (!cur_state.ins_alu.nop)
			{
				...
				if (cur_state.ins_alu.bad1)
				{
					...
					if (reg_lock[cur_state.ins_alu.rubbish1] == 0)
					{
						cur_state.ins_read_reg.value1 = registers[registerMap[cur_state.ins_alu.rubbish1]];
						cur_state.ins_alu.bad1 = 0;
					}
					else if (cur_state.ins_mem_write.destination == cur_state.ins_alu.rubbish1)
					{
						cur_state.ins_read_reg.value1 = new_state.ins_mem_read.mem_result_op;
						cur_state.ins_alu.bad1 = 0;

						// std::cout<<"Changing first value to: "<<cur_state.ins_mem_read.mem_result_op<<'\n';
					}
					else
					{
						new_state.ins_alu.nop = 0;

						goto stall1;
					}
				}
				if (cur_state.ins_alu.bad2)
				{
					if (reg_lock[cur_state.ins_alu.rubbish2] == 0)
					{
						cur_state.ins_alu.bad2 = 0;
						cur_state.ins_read_reg.value2 = registers[registerMap[cur_state.ins_alu.rubbish2]];
					}
					else if (cur_state.ins_mem_write.destination == cur_state.ins_alu.rubbish2)
					{
						// std::cout<<"Changing second value to: "<<cur_state.ins_mem_read.mem_result_op<<'\n';
						cur_state.ins_alu.bad2 = 0;
						cur_state.ins_read_reg.value2 = new_state.ins_mem_read.mem_result_op;
					}
					else
					{
						// stall!!
				...
				new_state.ins_mem_write.badmem = cur_state.ins_alu.badmem;
				new_state.ins_mem_write.rubbishmem = cur_state.ins_alu.rubbishmem;

				// std::cout << "Write enable mem on next cycle : " << new_state.ins_alu.write_mem << '\n';
			...
			if (!cur_state.ins_read_reg.nop)
			{
				...
				{
					if (reg_lock[cur_state.ins_dec2.reg1] || reg_lock[cur_state.ins_dec2.reg2])
					{

						if (reg_lock[cur_state.ins_dec2.reg1])
						{

							new_state.ins_alu.bad1 = 1;
							new_state.ins_alu.rubbish1 = cur_state.ins_dec2.reg1;
						}
						if (reg_lock[cur_state.ins_dec2.reg2])
						{

							// bypass
							new_state.ins_alu.bad2 = 1;
							new_state.ins_alu.rubbish2 = cur_state.ins_dec2.reg2;
						}
					}
					...
					reg_lock[new_state.ins_read_reg.destination]++;
				}
				if (i_type.find(new_state.ins_read_reg.operation) != i_type.end())
				{

					if (reg_lock[cur_state.ins_dec2.reg1] > 0)
					{
				...
				{
					if (reg_lock[cur_state.ins_dec2.reg1] || reg_lock[cur_state.ins_dec2.destination])
					{
						// std::cout<<"Stall or bypass?"<<" "<<cur_state.ins_dec2.reg1<<" " <<cur_state.ins_dec2.destination<<" "<<"\n";

						if (reg_lock[cur_state.ins_dec2.destination])
						{

							new_state.ins_alu.bad1 = 1;
							new_state.ins_alu.rubbish1 = cur_state.ins_dec2.destination;
						}
						if (reg_lock[cur_state.ins_dec2.reg1])
						{

							new_state.ins_alu.badmem = 1;
							new_state.ins_alu.rubbishmem = cur_state.ins_dec2.reg1;
						}
					}
					new_state.ins_read_reg.use_mem = 1;
				...
				// std::cout << "Values transfered to ALU: " << new_state.ins_read_reg.value1 << " " << new_state.ins_read_reg.value2 << " "<<new_state.ins_alu.bad1<<" " <<new_state.ins_alu.bad2<<'\n';
			}
			else
			{
				// std::cout << "RR is NOP\n";
		...
		stall2:
			...
			new_state.ins_read_reg = cur_state.ins_read_reg;
			new_state.ins_alu = cur_state.ins_alu;
			new_state.ins_mem_write = cur_state.ins_mem_write;
			new_state.ins_mem_read.nop = 1;
			cur_state = new_state;
			printRegistersAndMemoryDelta(clockCycles);
		}
