...
int
loaduvm(pde_t *pgdir, int va, struct inode *ip, uint offset, uint sz)
{
  uint i, n;
  int pa;
  pte_t *pte;
  // get the positive page offset of the va from the beginning of the page
  int first_va = PGROUNDDOWN(va);
  int pg_offset = va - first_va;

  // manually fill the first page, which might not be page aligned
  pte = walkpgdir(pgdir, (char*)first_va,0);
  pa = PTE_ADDR(*pte);
  if (pa == 0)
  ...
  for(i = PGSIZE; sz > 0; i += PGSIZE){
    pte = walkpgdir(pgdir, (char*)first_va + i,0);
    pa = PTE_ADDR(*pte);
    if(pa == 0)
      panic("loaduvm: address should exist");
