...
def classify(X , Theta):
    ...
    # print(prob_class)
    sums = np.sum(prob_class , axis = 1)
    # print(sums)
    class_9 = np.transpose([1 - sums])
    prob_class = np.concatenate((prob_class , class_9) , axis = 1)
    return np.transpose([np.argmax(prob_class , axis = 1) ]) + 1

def Evaluate(train_path, X_val , Y_val ,  test_path,  outputfile):
    ...
    op.to_csv(outputfile + '/classfication.csv' , index = False , header= False) 
