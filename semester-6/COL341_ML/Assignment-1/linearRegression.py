...
def find_MSE(X, Y, W, N):
    return (1 / (2 * N)) * (np.sum(np.square(Y - np.dot(X, W))))


def find_grad(X, Y, W, N):
    return (
        -(1 / N) * np.sum(np.dot((np.transpose(Y - np.dot(X, W))), X), axis=0)
    ).reshape((X.shape[1], 1))
...
def LinearReg(X , Y , X_val , Y_val ,test_path ,out_path,   alpha , Threshold , type):
    ...
    print(out_path+'/linearReg.csv')
    op.to_csv(out_path+'/linearReg.csv' , index=False , header=False)
    return W 